...
public class SQLParserTest {
	...
	private String[] testSelectSucceeds = {suspectsForCaseQuery, getPersonsForConvictionTypeString, "select A, B from C, (select * from C where c like 'lit') as B",
			categoriesForCaseQuery, getAllPersonsString, "select count(*), max(A.a) as maximum from B where c=40", 
			"select * from User where 'f' <=? and username='aaaaaa'"
			};
